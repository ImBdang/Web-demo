...
def build_control(msg: Dict[str, Any]) -> Optional[str]:
    """
    Gửi đúng format firmware đang parse:
      {"type":"cmd","mode":"trigger","enable":0/1}
      {"type":"cmd","mode":"repeat","work_time":..,"duration_time":..}
      {"type":"cmd","mode":"lock","lock_mode":0..3}
      {"type":"cmd","mode":"save","save_mode":0..2}

    -> HARD-SIMPLE: bỏ sender để khỏi rối (status sender là DEVICE_ID).
    """
    mode = msg.get("mode")
    base: Dict[str, Any] = {"type": "cmd", "mode": mode}

    try:
        if mode == "trigger":
            en = int(msg.get("enable"))
            if en not in (0, 1):
                return None
            base["enable"] = en
            ...
            lm = int(msg.get("lock_mode"))
            if not (0 <= lm <= 3):
                return None
            base["lock_mode"] = lm

        elif mode == "save":
            sm = int(msg.get("save_mode"))
            if not (0 <= sm <= 2):
                return None
            base["save_mode"] = sm
